...
export declare abstract class ZodType<Output, Def extends ZodTypeDef = ZodTypeDef, Input = Output> {
    ...
    spa: (x: unknown, params?: Partial<Pick<ParseParams, "path" | "errorMap" | "parentError" | "async">> | undefined) => Promise<{
        success: true;
        data: Output;
    } | {
        success: false;
    ...
    default<T extends Input, This extends this = this>(def: T): ZodOptional<This, true>;
    default<T extends () => Input, This extends this = this>(def: T): ZodOptional<This, true>;
    isOptional: () => boolean;
    isNullable: () => boolean;
}
...
export declare namespace objectUtil {
    type extendShape<U extends ZodRawShape, V extends ZodRawShape> = {
    ...
    type noNever<T extends ZodRawShape> = util.identity<{
...
export declare type baseObjectOutputType<Shape extends ZodRawShape> = util.flatten<objectUtil.addQuestionMarks<{
    [k in keyof Shape]: Shape[k]["_output"];
}>>;
export declare type objectOutputType<Shape extends ZodRawShape, Catchall extends ZodTypeAny> = ZodTypeAny extends Catchall ? baseObjectOutputType<Shape> : util.flatten<baseObjectOutputType<Shape> & {
    [k: string]: Catchall["_output"];
}>;
export declare type baseObjectInputType<Shape extends ZodRawShape> = util.flatten<objectUtil.addQuestionMarks<{
    [k in keyof Shape]: Shape[k]["_input"];
}>>;
export declare type objectInputType<Shape extends ZodRawShape, Catchall extends ZodTypeAny> = ZodTypeAny extends Catchall ? baseObjectInputType<Shape> : util.flatten<baseObjectInputType<Shape> & {
...
export declare class ZodObject<T extends ZodRawShape, UnknownKeys extends UnknownKeysParam = "strip", Catchall extends ZodTypeAny = ZodTypeAny, Output = objectOutputType<T, Catchall>, Input = objectInputType<T, Catchall>> extends ZodType<Output, ZodObjectDef<T, UnknownKeys, Catchall>, Input> {
     ...
     */
    merge: <Incoming extends AnyZodObject>(merging: Incoming) => ZodObject<T & Incoming["_shape"], UnknownKeys, Catchall>;
...
export declare type addDefaultToOptional<T extends ZodOptional<any, any>> = T extends ZodOptional<infer U, any> ? ZodOptional<U, true> : never;
export declare type removeDefaultFromOptional<T extends ZodOptional<any, any>> = T extends ZodOptional<infer U, any> ? ZodOptional<U, false> : never;
export declare type ZodOptionalType<T extends ZodTypeAny> = T extends ZodOptional<infer U, infer H> ? ZodOptional<U, H> : ZodOptional<T, false>;
export declare class ZodOptional<T extends ZodTypeAny, HasDefault extends boolean = false> extends ZodType<HasDefault extends true ? T["_output"] : T["_output"] | undefined, ZodOptionalDef<T>, T["_input"] | undefined> {
    _parse(ctx: ParseContext): any;
    unwrap(): T;
...
export declare type ZodNullableType<T extends ZodTypeAny> = T extends ZodNullable<infer U> ? ZodNullable<U> : ZodNullable<T>;
...
export declare type ZodFirstPartySchemaTypes = ZodString | ZodNumber | ZodBigInt | ZodBoolean | ZodDate | ZodUndefined | ZodNull | ZodAny | ZodUnknown | ZodNever | ZodVoid | ZodArray<any> | ZodObject<any> | ZodUnion<any> | ZodTuple | ZodRecord | ZodMap | ZodSet | ZodFunction<any, any> | ZodLazy<any> | ZodLiteral<any> | ZodEnum<any> | ZodEffects<any> | ZodNativeEnum<any> | ZodOptional<any> | ZodNullable<any> | ZodPromise<any>;
...
declare const ostring: () => ZodOptional<ZodString, false>;
declare const onumber: () => ZodOptional<ZodNumber, false>;
declare const oboolean: () => ZodOptional<ZodBoolean, false>;
export { anyType as any, arrayType as array, bigIntType as bigint, booleanType as boolean, dateType as date, effectsType as effect, enumType as enum, functionType as function, instanceOfType as instanceof, lazyType as lazy, literalType as literal, mapType as map, nativeEnumType as nativeEnum, neverType as never, nullType as null, nullableType as nullable, numberType as number, objectType as object, oboolean, onumber, optionalType as optional, ostring, promiseType as promise, recordType as record, setType as set, strictObjectType as strictObject, stringType as string, effectsType as transformer, tupleType as tuple, undefinedType as undefined, unionType as union, unknownType as unknown, voidType as void, };
